...
array.forEach((item, index) => {
    ...
    if (index % 2 == 0 && index > 0) {
        const cloneTemp = {...tempObj};
        userList.push(cloneTemp);
    }

});

const countResult = userList.filter((user, index) => Number(user.age) == 32);

countResult.forEach((data) => {
    fs.appendFile('./output.txt', data.key + "\n", (err) => {
